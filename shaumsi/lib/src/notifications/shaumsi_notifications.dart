...
class ShauMsiNotifications {
  ShauMsiNotifications._();

  static final ShauMsiNotifications instance = ShauMsiNotifications._();

  static const String _defaultChannelId = 'shaumsi_important_dates_default';
  static const String _defaultChannelName = 'Datas importantes';
  static const String _customSoundChannelId =
      'shaumsi_important_dates_shaumsi_tone';
  static const String _customSoundChannelName =
      'Datas importantes (som ShauMsi)';
  static const String _channelDescription =
      'Lembretes locais para datas importantes do Shauku ya msimu.';
  static const String _customSoundResourceName =
      ImportantDate.notificationSoundShauMsi;
  static const Duration _minimumLeadTime = Duration(seconds: 5);
  ...
  Future<void> syncImportantDateNotifications(List<ImportantDate> dates) async {
    ...
    for (final date in dates) {
      try {
        ...
        _logger.error(
          'ShauMsiNotifications',
          'Falha ao sincronizar notificação da data (id=${date.id}).',
          error: error,
  ...
  Future<void> scheduleForImportantDate(ImportantDate date) async {
    ...
    final fallbackDetails = _defaultDetails();
    final hasCustomSound =
        date.notificationSound == ImportantDate.notificationSoundShauMsi;
    ...
    for (final rule in rules) {
      ...
      await _scheduleWithFallback(
        ...
        trigger: nextTrigger,
        details: details,
        fallbackDetails: hasCustomSound ? fallbackDetails : null,
  ...
  tz.TZDateTime? _computeNextTrigger(
    ...
    final minTrigger = now.add(_minimumLeadTime);
  ...
  Future<void> _scheduleWithFallback({
    ...
    try {
      await _scheduleWithModeFallback(
      ...
      if (_isInvalidSoundError(error) && fallbackDetails != null) {
        _logger.warning(
          'ShauMsiNotifications',
          'Som personalizado indisponível; reagendando com som padrão.',
          error: error,
        );
        await _scheduleWithModeFallback(
          notificationId: notificationId,
  ...
  Future<void> _scheduleWithModeFallback({
    required int notificationId,
    required String title,
    required String body,
    required tz.TZDateTime trigger,
  ...
  bool _isInvalidSoundError(PlatformException error) {
    return error.code.trim().toLowerCase() == 'invalid_sound';
  }
}
